...
logging.basicConfig(filename='libStark.log', level=logging.INFO)
...
def TestingNumericalSolver4():
    ...
    logging.info("Attempting numerical solution ...")
    roots=[]
    ...
    logging.info(np.shape(Efield_AU))
    logging.info(np.shape(roots))
    logging.info(roots[0][:])
    return roots
...
def ComputeFloquetBandStructure(filename, nb_atoms, Z_electrons, kpoints, unocc_states, MPI_number, Efield_SI, wavelength_SI, num_time_steps, eigenvalues, matrixelements, Complex_valued_ME=False, field_polarization_dir=0, spin_occupation=2, verbose=1): #{{{
    ...
    if(verbose==1):
        logging.info(Header+"** Info: side of the matrix")
        logging.info(Header+"         "+str(np.size(H_GS[0])))
        logging.info(Header+"Matrix_side: "+str(matrix_side)+" Z_el: "+str(Z_electrons)+" unocc: "+str(unocc_states))
    for i in np.arange(0,np.size(H_GS[0])):
        H_GS[i,i]=eigenvalues[i]
    if(verbose==1):
        logging.debug("Info: matrix of eigenvalues for the selected k-point. Matrix shape is "+str(np.shape(H_GS)))
    ...
    if(verbose==1):
        logging.info("Info: H(t)=H_GS + dipolar_matrix * Efield(t)")
        logging.info("Info: shape of H_GS: "+str(np.shape(H_GS)))
        logging.info("Info: shape of matrixelements: "+str(np.shape(matrixelements)))
    ...
    if(verbose==1):
        logging.info("Info: shape of H(t): "+str(np.shape(H0)))
        logging.info("Info: shape of RabiFreq(t): "+str(np.shape(Rabi0)))
    ...
    if(verbose==1):
        logging.info(Header+"Ideal photon energies for "+str(Efield_SI*1E-9)+" V/nm: \n"+str(list(set(OmegaCutOff_eV_sorted)))+" (eV)")
        logging.info(Header+"Ideal photon wavelengths for "+str(Efield_SI*1E-9)+" V/nm: \n"+str(list(set(OmegaCutOff_m_sorted)))+" (m)")

    #  wavelength (m) = h * c / (E(eV) * e)
    #print H_Floquet_mn 
    
    if(verbose==1):
        logging.info("Info: element (m,n) of Floquet matrix constructed with success.")
        logging.info("Info: Now, one has to write the full matrix.")

    if(verbose==1):
        logging.info("Info: shape of element [m,n] of the matrix we want to write: "+str(np.shape(H_Floquet_mn)))
    H_Floquet_shape = (MPI_number*2+1) * (matrix_side)
    H_Floquet=np.zeros((H_Floquet_shape, H_Floquet_shape))*1j
    if(verbose==1): 
        logging.info("Info: shape of the complete Floquet matrix")
        logging.info(H_Floquet_shape, H_Floquet_shape)
        logging.info(np.shape(H_Floquet))
