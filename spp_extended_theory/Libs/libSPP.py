...
def SPPactiveInterfaces(dbarray, comment):#{{{
  ...
  SPParray = np.empty((0,23)) #, dtype='|S30')
    ...
    for k in dbarray:
      ...
      if ((wavelength1 == wavelength2) and ConditionOnGap): #we must consider same wavelength, otherwise there is no meaning, but we also select only metallic substrates
        ...
        if(LevelOfSPPaccuracy==0):
          ...
          sys.exit()
        if(Condition):
          kspp = betaSPP(wavelength1, eps1, eps2)
          Period=(period(kspp)/lengthunit)
          ...
          SPPdecayLength=DecayLengthSPP(kspp)/lengthunit
          ...
          LifeTime = LifeTimeRaether(kspp, eps2, eps1)

          # uses Hohenau / Jackson definition of group velocity for non-absorbing materials
          # SPPgroupVelocityHohenau = HohenauGroupVelocity(omega(wavelength1), kspp)
          # LifeTime = LifeTimeVg(kspp, HohenauGroupVelocity(wavelength1, kspp)) #lifetime of SPP at interface1-2
        ...
        if(Condition):
          ...
          SPParray = np.vstack((SPParray, [Material1, Material2, Wavelength, OldSPPactiveBool, NewSPPactiveBool, #5
