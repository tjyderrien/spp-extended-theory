"""                                                             
   ...
   VERSION
     ...
     $Id: interspectus.py 46 2011-04-14 10:25:49Z hambach $
   ...
   DESCRIPTION
     ...
     function prepare_interpolation() will already
       ...
       A normalize the input-curves f(x) for each q and x           (i)
       B compute the integrals F(x) for each q                      (ii)
       C inverte this function to get x(q) for each F
       D store x(q,F) in the privat member variable F_inverted
     ...
     Second, each time the user calls get_spectrum(), we
       ...
       I re-normalize output-curves f(x) for each q_new and x

   IMPLEMENTATION
   
     We need to interpolate the curves (using cubic splines):
     ...
     Numerical methods:
       ...
       integration:     Simpson's rule/Trapezoid rule
       differentiation: 5-point finite differences method
...
#__date__ = "$Date: 2011-04-12 20:35:20 +0200 (Tue, 12 Apr 2011) $"
...
class FamilyOfCurves:
  ...
  def plot(self,ax=None,title=None,legend=True):
    ...
    plt.xlabel(self.xdesc); plt.ylabel(self.fdesc);
...
class Interpolator:
  ...
  def __init__(self,q,x,f,wfunc=None,debug=False,k=3):
    """
    q    ... 1D-array of shape (q.size)
    x    ... 2D-array of shape (q.size,x.size)
    f    ... 2D-array of shape (q.size,x.size) 
    wfunc... (optional) function reference to w(q,x)
    debug... (optional) boolean for debugging info's
    k    ... (optional) order of spline interpolation along q
    """
  ...
  def __prepare_interpolation(self):
    ...
    def integrate(f,x):
      ...
      for i in range(x.size):
        #Simpsons rule seems to be problematic for f(x)~0
    ...
    if self.__debug:
      ...
      FamilyOfCurves(q,x,f).plot(ax=ax, title="Debug: (B) Integration");
      FamilyOfCurves(q,x,F,qdesc="q",xdesc="x",fdesc="F").plot(ax=ax,legend=False);
      

    # -- (C) inversion F(q,x) -> xi(q,Fi) -------------------------------------
    Fi=np.unique(np.concatenate((F[0],F[-1]))); # adaptive grid for new F vals
    Fi.sort();
  ...
  def get_spectrum(self,q_new,x_new=None):
    ...
    def differentiate3(F,x):
      ...
      f=np.empty(len(x));
      f[1:-1]=(F[2:]-F[0:-2])/(x[1]-x[0]); # vectorized for speed up
      f[0]=f[-1]=0.
      return f;

    def differentiate5(F,x):
      "five point method (error in O(h**3))"
      f=np.empty(len(x));
    ...
    for iqn in range(q_new.size):
      f_new[iqn]=differentiate5(F_new[iqn],x_new);
    

    # -- (I) re-normalization: f_new -> f_new / w_new -------------------------
